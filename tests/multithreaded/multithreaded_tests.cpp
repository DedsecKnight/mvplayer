...
struct mock_processor
  ...
  void operator()(const mock_event<int32_t>& e) override {
    event_received = true;
    received_payload.push_back(e.x);
  }
...
struct variadic_mock_processor
  ...
  void operator()(const mock_event<int32_t>& e) override {
    event_received = true;
    received_payload.emplace_back(e.x);
  }
  void operator()(const mock_event<double>& e) override {
    event_received = true;
    received_payload.emplace_back(e.x);
  }
  void on_startup(std::span<char* const>) const noexcept {}
